...
class Engine:
	def __init__(self):
		...
		self.rate_limiter = RateLimiter(default_limit_per_hour=43, enabled=True)
...
def detonate(email_file_path, smtp_file_path, message_file_path, subject):
	...
	# Prompt for sender name with retries
	sender_name = None
	max_retries = 3
	for attempt in range(max_retries):
		sender_name = simpledialog.askstring(
			"Input", 
			"Enter your preferred sender name for the email From header:",
			parent=root
		)
		if sender_name and sender_name.strip():
			engine.logger.info("Sender name set to: %s", sender_name.strip())
			break
		engine.logger.warning("Invalid or empty sender name provided on attempt %d", attempt + 1)
		if attempt < max_retries - 1:
			messagebox.showwarning("Warning", "Sender name cannot be empty. Please try again.")
		else:
			sender_name = "Email Sender"
			engine.logger.info("No valid sender name provided after %d attempts. Using default: %s", max_retries, sender_name)
